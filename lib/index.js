...
const customProviders = {
  'flic.kr': require('./providers/flic.kr'),
  'google.com': require('./providers/google.com'),
  'vk.com': require('./providers/vk.com')
}
...
Unshort.prototype._expand = async function (origUrl) {
  ...
  for (; nestingLeft >= 0; nestingLeft--) {
    ...
    const url_normalized = mdurl.format(u)
    ...
    const domainConfig = this._domains.find(dc => dc._compiledMatch.exec(url_normalized))
...
Unshort.prototype._defaultFetch = async function (url, options) {
  ...
  if (res.statusCode >= 200 && res.statusCode < 300) {
    ...
    if (options.select) {
      ...
      const el = $(body).find(options.select)
      const result = el.attr('src') || el.attr('href')
