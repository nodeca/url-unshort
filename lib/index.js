...
Unshort.prototype.request = function (url, options) {
  ...
  //
  if (typeof o.followRedirect === 'undefined') o.followRedirect = false;

  // Set default timeout to 10 seconds
  //
  if (typeof o.timeout === 'undefined') o.timeout = 10000;

  // Override default - 5 retries not needed
  if (typeof o.retries === 'undefined') o.retries = 1;

  // Set up a default user-agent
  //
  if (!o.headers) { o.headers = {}; }

  if (Object.keys(o.headers).map(k => k.toLowerCase).indexOf('user-agent') === -1) {
    o.headers['User-Agent'] = 'Link expander, https://github.com/nodeca/url-unshort';
  }

  // Specials to restrict download size
  o.maxBuffer = 1024 * 100; // if reply > 100K, consider as not redirect.
...
Unshort.prototype._defaultFetch = Promise.coroutine(function* (url, options) {
  ...
  try {
    ...
    throw new UnshortError(`Remote server error, code ${e.statusCode}`, 'EHTTP', e.statusCode);
  }
  ...
  throw new UnshortError(`Remote server error, code ${res.statusCode}`, 'EHTTP', res.statusCode);
});
