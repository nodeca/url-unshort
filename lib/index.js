...
Unshort.prototype._expand = function (origUrl, url, nestingLeft, callback) {
  ...
  // Something found - normalize url & run additional checks.
  ...
  //
  // If we are here - fetch needed. At top level try cache first.
  // On recursive calls skip cache.
  //
  var cache_get = (nestingLeft === self.nesting) ? self.cache.get : dummy_cache;

  cache_get(origUrl, function (err, result) {
    if (err) {
      callback(err);
      return;
    }
    ...
    domainConfig.fetch.call(self, url_normalized, domainConfig, function (err, result) {
      ...
      // Strange special case, empty result but not error - stop & remenber
      // url in cache.
      if (!result) {
        self.cache.set(origUrl, url, function (err) {
          callback(err, null);
        });
        return;
      }
      ...
      self._expand(origUrl, result, nestingLeft - 1, function (err, recursive) {
        ...
        self.cache.set(origUrl, result, function (err) {
          if (err) {
            callback(err);
            return;
          }
